...
var _ = Describe("Commands", func() {
	...
	Describe("lists", func() {
		...
		It("should LMPop", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(keyVals.Key).To(Equal("list1"))
			Expect(keyVals.Values).To(Equal([]string{"five", "four", "three"}))

			keyVals, err = client.LMPop(ctx, "right", 3, "list1", "list2").Result()
			Expect(err).NotTo(HaveOccurred())
			Expect(keyVals.Key).To(Equal("list1"))
			Expect(keyVals.Values).To(Equal([]string{"one", "two"}))

			keyVals, err = client.LMPop(ctx, "left", 1, "list1", "list2").Result()
			Expect(err).NotTo(HaveOccurred())
			Expect(keyVals.Key).To(Equal("list2"))
			Expect(keyVals.Values).To(Equal([]string{"e"}))

			keyVals, err = client.LMPop(ctx, "right", 10, "list1", "list2").Result()
			Expect(err).NotTo(HaveOccurred())
			Expect(keyVals.Key).To(Equal("list2"))
			Expect(keyVals.Values).To(Equal([]string{"a", "b", "c", "d"}))
