...
var _ = Describe("ScanIterator", func() {
	...
	var seed = func(n int) error {
		...
		for i := 1; i <= n; i++ {
			pipe.Set(context.Background(), fmt.Sprintf("K%02d", i), "x", 0).Err()
		}
		_, err := pipe.Exec()
		return err
	}
	...
	var extraSeed = func(n int, m int) error {
		pipe := client.Pipeline()
		for i := 1; i <= m; i++ {
			pipe.Set(context.Background(), fmt.Sprintf("A%02d", i), "x", 0).Err()
		}
		for i := 1; i <= n; i++ {
			pipe.Set(context.Background(), fmt.Sprintf("K%02d", i), "x", 0).Err()
		}
	...
	var hashSeed = func(n int) error {
		pipe := client.Pipeline()
		for i := 1; i <= n; i++ {
			pipe.HSet(context.Background(), hashKey, fmt.Sprintf("K%02d", i), "x").Err()
		}
		_, err := pipe.Exec()
		return err
	}
	...
	It("should scan across empty DBs", func() {
		iter := client.Scan(context.Background(), 0, "", 10).Iterator()
	...
	It("should hscan across multiple pages", func() {
		...
		var vals []string
		iter := client.HScan(context.Background(), hashKey, 0, "", 10).Iterator()
		for iter.Next() {
			vals = append(vals, iter.Val())
		}
		Expect(iter.Err()).NotTo(HaveOccurred())
	...
	It("should scan with match", func() {
		...
		var vals []string
		iter := client.Scan(context.Background(), 0, "K*2*", 10).Iterator()
		for iter.Next() {
			vals = append(vals, iter.Val())
		}
		Expect(iter.Err()).NotTo(HaveOccurred())
	...
	It("should scan with match across empty pages", func() {
		...
		var vals []string
		iter := client.Scan(context.Background(), 0, "K*", 1).Iterator()
		for iter.Next() {
			vals = append(vals, iter.Val())
		}
		Expect(iter.Err()).NotTo(HaveOccurred())
