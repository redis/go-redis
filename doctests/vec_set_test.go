...
func ExampleClient_vectorset() {
	...
	// STEP_START vadd
	res1, err := rdb.VAdd(ctx, "points", "pt:A", &redis.VectorValues{Val: []float64{1.0, 1.0}}).Result()
	if err != nil {
		panic(err)
	}
	fmt.Println(res1) // >>> true

	res2, err := rdb.VAdd(ctx, "points", "pt:B", &redis.VectorValues{Val: []float64{-1.0, -1.0}}).Result()
	if err != nil {
		panic(err)
	}
	fmt.Println(res2) // >>> true

	res3, err := rdb.VAdd(ctx, "points", "pt:C", &redis.VectorValues{Val: []float64{-1.0, 1.0}}).Result()
	if err != nil {
		panic(err)
	}
	fmt.Println(res3) // >>> true

	res4, err := rdb.VAdd(ctx, "points", "pt:D", &redis.VectorValues{Val: []float64{1.0, -1.0}}).Result()
	if err != nil {
		panic(err)
	}
	fmt.Println(res4) // >>> true

	res5, err := rdb.VAdd(ctx, "points", "pt:E", &redis.VectorValues{Val: []float64{1.0, 0.0}}).Result()
	if err != nil {
		panic(err)
	}
	...
	fmt.Println(res15) // >>> {"description":"First point added","name":"Point A"}
	...
	// STEP_START vrem
	res18, err := rdb.VAdd(ctx, "points", "pt:F", &redis.VectorValues{Val: []float64{0.0, 0.0}}).Result()
	if err != nil {
		panic(err)
	}
	...
	res22, err := rdb.VSim(ctx, "points", &redis.VectorValues{Val: []float64{0.9, 0.1}}).Result()
	if err != nil {
		panic(err)
	}
	...
	fmt.Println(res23)
	// >>> [{pt:A 1} {pt:E 0.8535534143447876} {pt:C 0.5} {pt:D 0.5}]
	// STEP_END

	// STEP_START vsim_filter
	// Set attributes for filtering
	res24, err := rdb.VSetAttr(ctx, "points", "pt:A", map[string]interface{}{
		"size":  "large",
		"price": 18.99,
	}).Result()
	if err != nil {
	...
	res25, err := rdb.VSetAttr(ctx, "points", "pt:B", map[string]interface{}{
		"size":  "large",
		"price": 35.99,
	}).Result()
	if err != nil {
	...
	res26, err := rdb.VSetAttr(ctx, "points", "pt:C", map[string]interface{}{
		"size":  "large",
		"price": 25.99,
	}).Result()
	if err != nil {
	...
	res27, err := rdb.VSetAttr(ctx, "points", "pt:D", map[string]interface{}{
		"size":  "small",
		"price": 21.00,
	}).Result()
	if err != nil {
	...
	res28, err := rdb.VSetAttr(ctx, "points", "pt:E", map[string]interface{}{
		"size":  "small",
		"price": 17.75,
	}).Result()
	if err != nil {
	...
	res29, err := rdb.VSimWithArgs(ctx, "points", &redis.VectorRef{Name: "pt:A"}, &redis.VSimArgs{Filter: `.size == "large"`}).Result()
	if err != nil {
		panic(err)
	}
	fmt.Println(res29) // >>> [pt:A pt:C pt:B]

	res30, err := rdb.VSimWithArgs(ctx, "points", &redis.VectorRef{Name: "pt:A"}, &redis.VSimArgs{Filter: `.size == "large" && .price > 20.00`}).Result()
...
func ExampleClient_vectorset_quantization() {
	...
	res1, err := rdb.VAddWithArgs(ctx, "quantSetQ8", "quantElement", vecQ, &redis.VAddArgs{
		Q8: true,
	}).Result()
	if err != nil {
		panic(err)
	...
	fmt.Printf("Q8 embedding: %v\n", embQ8) // >>> Q8 embedding: [1.2621850967407227 1.9582309722900391]

	// Add with NOQUANT option
	res2, err := rdb.VAddWithArgs(ctx, "quantSetNoQ", "quantElement", vecQ, &redis.VAddArgs{
		NoQuant: true,
	}).Result()
	if err != nil {
		panic(err)
	...
	fmt.Printf("NOQUANT embedding: %v\n", embNoQ) // >>> NOQUANT embedding: [1.262185 1.958231]

	// Add with BIN quantization
	res3, err := rdb.VAddWithArgs(ctx, "quantSetBin", "quantElement", vecQ, &redis.VAddArgs{
		Bin: true,
	}).Result()
	if err != nil {
		panic(err)
...
func ExampleClient_vectorset_dimension_reduction() {
	...
	fmt.Printf("Dimension without reduction: %d\n", dim1) // >>> Dimension without reduction: 300

	// Add with reduction to 100 dimensions
	res2, err := rdb.VAddWithArgs(ctx, "setReduced", "element", vecLarge, &redis.VAddArgs{
		Reduce: 100,
	}).Result()
	if err != nil {
		panic(err)
