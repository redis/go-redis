...
var _ = Describe("RedisTimeseries commands", Label("timeseries"), func() {
	...
	It("should TSCreate and TSCreateArgs", Label("timeseries", "tscreate", "tscreateargs"), func() {
		...
		result, err = client.TSCreateArgs(ctx, "ts-cs-1", opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo("OK"))
		...
		for _, dup := range duplicate_policies {
			...
			result, err = client.TSCreateArgs(ctx, keyName, opt).Result()
			Expect(err).NotTo(HaveOccurred())
			Expect(result).To(BeEquivalentTo("OK"))
	...
	It("should TSAdd and TSAddArgs", Label("timeseries", "tsadd", "tsaddargs"), func() {
		...
		// Test chunk size
		opt = &redis.TSOptions{ChunkSize: 128}
		result, err = client.TSAddArgs(ctx, "ts-cs-1", 1, 10, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo(1))
		resultInfo, err = client.TSInfo(ctx, "ts-cs-1").Result()
		Expect(err).NotTo(HaveOccurred())
		...
		result, err = client.TSAddArgs(ctx, "tsal-1", 1, 5, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo(1))
		result, err = client.TSAddArgs(ctx, "tsal-1", 1, 10, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo(1))
		...
		result, err = client.TSAddArgs(ctx, "tsaf-1", 1, 5, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo(1))
		result, err = client.TSAddArgs(ctx, "tsaf-1", 1, 10, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo(1))
		...
		result, err = client.TSAddArgs(ctx, "tsam-1", 1, 5, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo(1))
		result, err = client.TSAddArgs(ctx, "tsam-1", 1, 10, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo(1))
		...
		result, err = client.TSAddArgs(ctx, "tsami-1", 1, 5, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo(1))
		result, err = client.TSAddArgs(ctx, "tsami-1", 1, 10, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo(1))
	...
	It("should TSAlter and TSAlterArgs", Label("timeseries", "tsalter", "tsalterargs"), func() {
		result, err := client.TSCreate(ctx, "1").Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo("OK"))
		...
		resultAlter, err := client.TSAlterArgs(ctx, "1", opt).Result()
		Expect(err).NotTo(HaveOccurred())
	...
	It("should TSCreateRule and TSDeleteRule", Label("timeseries", "tscreaterule", "tsdeleterule"), func() {
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo("OK"))
		result, err = client.TSCreateRule(ctx, "1", "2", "avg", 100).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo("OK"))
		for i := 0; i < 50; i++ {
	...
	It("should TSIncrBy, TSIncrByArgs, TSDecrBy and TSDecrByArgs", Label("timeseries", "tsincrby", "tsdecrby", "tsincrbyargs", "tsdecrbyargs"), func() {
		for i := 0; i < 100; i++ {
		...
		_, err = client.TSIncrByArgs(ctx, "2", 1.5, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		_, err = client.TSIncrByArgs(ctx, "2", 2.25, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		_, err = client.TSDecrByArgs(ctx, "2", 1.5, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		_, err = client.TSIncrByArgs(ctx, "3", 10, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		opt = &redis.TSIncrDecrOptions{ChunkSize: 128}
		_, err = client.TSDecrByArgs(ctx, "4", 10, opt).Result()
		Expect(err).NotTo(HaveOccurred())
	...
	It("should TSGet", Label("timeseries", "tsget"), func() {
		opt := &redis.TSOptions{DuplicatePolicy: "max"}
		resultGet, err := client.TSAddArgs(ctx, "foo", 2265985, 151, opt).Result()
		Expect(err).NotTo(HaveOccurred())
	...
	It("should TSGet Latest", Label("timeseries", "tsgetlatest"), func() {
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(resultGet).To(BeEquivalentTo("OK"))
		resultGet, err = client.TSCreateRule(ctx, "tsgl-1", "tsgl-2", "sum", 10).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultGet).To(BeEquivalentTo("OK"))
		...
		result, errGet = client.TSGetArgs(ctx, "tsgl-2", &redis.TSGetOptions{Latest: true}).Result()
		Expect(errGet).NotTo(HaveOccurred())
	...
	It("should TSMGet and TSMGetArgs", Label("timeseries", "tsmget", "tsmgetargs"), func() {
		opt := &redis.TSOptions{Labels: map[string]string{"Test": "This"}}
		resultCreate, err := client.TSCreateArgs(ctx, "a", opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultCreate).To(BeEquivalentTo("OK"))
		opt = &redis.TSOptions{Labels: map[string]string{"Test": "This", "Taste": "That"}}
		resultCreate, err = client.TSCreateArgs(ctx, "b", opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultCreate).To(BeEquivalentTo("OK"))
		...
		result, err = client.TSMGetArgs(ctx, []string{"Test=This"}, mgetOpt).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		resultCreate, err = client.TSCreateArgs(ctx, "d", opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultCreate).To(BeEquivalentTo("OK"))
		resultCreateRule, err := client.TSCreateRule(ctx, "c", "d", "sum", 10).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		result, err = client.TSMGetArgs(ctx, []string{"is_compaction=true"}, mgetOpt).Result()
		Expect(err).NotTo(HaveOccurred())
	...
	It("should TSRange, TSRangeArgs", Label("timeseries", "tsrange", "tsrangeargs"), func() {
		for i := 0; i < 100; i++ {
			_, err := client.TSAdd(ctx, "a", i, float64(i%7)).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		result, err = client.TSRangeArgs(ctx, "a", 0, 500, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(len(result)).To(BeEquivalentTo(2))
		opt = &redis.TSRangeOptions{Aggregator: "count", BucketDuration: 10, Align: "+"}
		result, err = client.TSRangeArgs(ctx, "a", 0, 10, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo([]redis.TSTimestampValue{{Timestamp: 0, Value: 10}, {Timestamp: 10, Value: 1}}))
		opt = &redis.TSRangeOptions{Aggregator: "count", BucketDuration: 10, Align: "5"}
		result, err = client.TSRangeArgs(ctx, "a", 0, 10, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo([]redis.TSTimestampValue{{Timestamp: 0, Value: 5}, {Timestamp: 5, Value: 6}}))
		opt = &redis.TSRangeOptions{Aggregator: "twa", BucketDuration: 10}
		result, err = client.TSRangeArgs(ctx, "a", 0, 10, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(resultCreate).To(BeEquivalentTo("OK"))
		resultRule, err := client.TSCreateRule(ctx, "t1", "t2", "sum", 10).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		resultRange, err = client.TSRangeArgs(ctx, "t2", 0, 10, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		opt = &redis.TSRangeOptions{Aggregator: "max", Align: 0, BucketDuration: 10}
		resultRange, err = client.TSRangeArgs(ctx, "t3", 0, 100, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		opt = &redis.TSRangeOptions{Aggregator: "max", Align: 0, BucketDuration: 10, BucketTimestamp: "+"}
		resultRange, err = client.TSRangeArgs(ctx, "t3", 0, 100, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		opt = &redis.TSRangeOptions{Aggregator: "max", Align: 0, BucketDuration: 10}
		resultRange, err = client.TSRangeArgs(ctx, "t4", 0, 100, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultRange[0]).To(BeEquivalentTo(redis.TSTimestampValue{Timestamp: 10, Value: 4}))
		Expect(len(resultRange)).To(BeEquivalentTo(3))

		opt = &redis.TSRangeOptions{Aggregator: "max", Align: 0, BucketDuration: 10, Empty: true}
		resultRange, err = client.TSRangeArgs(ctx, "t4", 0, 100, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultRange[0]).To(BeEquivalentTo(redis.TSTimestampValue{Timestamp: 10, Value: 4}))
	...
	It("should TSRevRange, TSRevRangeArgs", Label("timeseries", "tsrevrange", "tsrevrangeargs"), func() {
		for i := 0; i < 100; i++ {
			_, err := client.TSAdd(ctx, "a", i, float64(i%7)).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		opt := &redis.TSRevRangeOptions{Aggregator: "avg", BucketDuration: 10}
		result, err = client.TSRevRangeArgs(ctx, "a", 0, 500, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		opt = &redis.TSRevRangeOptions{Aggregator: "count", BucketDuration: 10, Align: "+"}
		result, err = client.TSRevRangeArgs(ctx, "a", 0, 10, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo([]redis.TSTimestampValue{{Timestamp: 10, Value: 1}, {Timestamp: 0, Value: 10}}))

		opt = &redis.TSRevRangeOptions{Aggregator: "count", BucketDuration: 10, Align: "1"}
		result, err = client.TSRevRangeArgs(ctx, "a", 0, 10, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(BeEquivalentTo([]redis.TSTimestampValue{{Timestamp: 1, Value: 10}, {Timestamp: 0, Value: 1}}))

		opt = &redis.TSRevRangeOptions{Aggregator: "twa", BucketDuration: 10}
		result, err = client.TSRevRangeArgs(ctx, "a", 0, 10, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		resultRange, err = client.TSRevRangeArgs(ctx, "t2", 0, 10, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultRange[0]).To(BeEquivalentTo(redis.TSTimestampValue{Timestamp: 10, Value: 8}))
		resultRange, err = client.TSRevRangeArgs(ctx, "t2", 0, 9, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultRange[0]).To(BeEquivalentTo(redis.TSTimestampValue{Timestamp: 0, Value: 4}))
		// Test Bucket Timestamp
		resultCreate, err = client.TSCreate(ctx, "t3").Result()
		...
		opt = &redis.TSRevRangeOptions{Aggregator: "max", Align: 0, BucketDuration: 10}
		resultRange, err = client.TSRevRangeArgs(ctx, "t3", 0, 100, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultRange[0]).To(BeEquivalentTo(redis.TSTimestampValue{Timestamp: 70, Value: 5}))
		Expect(len(resultRange)).To(BeEquivalentTo(3))

		opt = &redis.TSRevRangeOptions{Aggregator: "max", Align: 0, BucketDuration: 10, BucketTimestamp: "+"}
		resultRange, err = client.TSRevRangeArgs(ctx, "t3", 0, 100, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		opt = &redis.TSRevRangeOptions{Aggregator: "max", Align: 0, BucketDuration: 10}
		resultRange, err = client.TSRevRangeArgs(ctx, "t4", 0, 100, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultRange[0]).To(BeEquivalentTo(redis.TSTimestampValue{Timestamp: 70, Value: 5}))
		Expect(len(resultRange)).To(BeEquivalentTo(3))

		opt = &redis.TSRevRangeOptions{Aggregator: "max", Align: 0, BucketDuration: 10, Empty: true}
		resultRange, err = client.TSRevRangeArgs(ctx, "t4", 0, 100, opt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultRange[0]).To(BeEquivalentTo(redis.TSTimestampValue{Timestamp: 70, Value: 5}))
		Expect(len(resultRange)).To(BeEquivalentTo(7))
	...
	It("should TSMRange and TSMRangeArgs", Label("timeseries", "tsmrange", "tsmrangeargs"), func() {
		createOpt := &redis.TSOptions{Labels: map[string]string{"Test": "This", "team": "ny"}}
		resultCreate, err := client.TSCreateArgs(ctx, "a", createOpt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultCreate).To(BeEquivalentTo("OK"))
		createOpt = &redis.TSOptions{Labels: map[string]string{"Test": "This", "Taste": "That", "team": "sf"}}
		resultCreate, err = client.TSCreateArgs(ctx, "b", createOpt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultCreate).To(BeEquivalentTo("OK"))
		...
		result, err = client.TSMRangeArgs(ctx, 0, 200, []string{"Test=This"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		mrangeOpt = &redis.TSMRangeOptions{Aggregator: "avg", BucketDuration: 10}
		result, err = client.TSMRangeArgs(ctx, 0, 500, []string{"Test=This"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(len(result)).To(BeEquivalentTo(2))
		...
		result, err = client.TSMRangeArgs(ctx, 0, 3, []string{"Test=This"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		// Test Align
		mrangeOpt = &redis.TSMRangeOptions{Aggregator: "count", BucketDuration: 10, Align: "-"}
		result, err = client.TSMRangeArgs(ctx, 0, 10, []string{"team=ny"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result["a"][2]).To(BeEquivalentTo([]interface{}{[]interface{}{int64(0), 10.0}, []interface{}{int64(10), 1.0}}))

		mrangeOpt = &redis.TSMRangeOptions{Aggregator: "count", BucketDuration: 10, Align: 5}
		result, err = client.TSMRangeArgs(ctx, 0, 10, []string{"team=ny"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
	...
	It("should TSMRangeArgs Latest", Label("timeseries", "tsmrangeargs", "tsmrangelatest"), func() {
		...
		resultCreateRule, err := client.TSCreateRule(ctx, "a", "b", "sum", 10).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultCreateRule).To(BeEquivalentTo("OK"))
		resultCreateRule, err = client.TSCreateRule(ctx, "c", "d", "sum", 10).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		result, err := client.TSMRangeArgs(ctx, 0, 10, []string{"is_compaction=true"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
	...
	It("should TSMRevRange and TSMRevRangeArgs", Label("timeseries", "tsmrevrange", "tsmrevrangeargs"), func() {
		createOpt := &redis.TSOptions{Labels: map[string]string{"Test": "This", "team": "ny"}}
		resultCreate, err := client.TSCreateArgs(ctx, "a", createOpt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultCreate).To(BeEquivalentTo("OK"))
		...
		result, err = client.TSMRevRangeArgs(ctx, 0, 200, []string{"Test=This"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(len(result["a"][2].([]interface{}))).To(BeEquivalentTo(10))
		// Test Aggregation and BucketDuration
		for i := 0; i < 100; i++ {
		...
		mrangeOpt = &redis.TSMRevRangeOptions{Aggregator: "avg", BucketDuration: 10}
		result, err = client.TSMRevRangeArgs(ctx, 0, 500, []string{"Test=This"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(len(result)).To(BeEquivalentTo(2))
		Expect(len(result["a"][2].([]interface{}))).To(BeEquivalentTo(20))
		Expect(result["a"][0]).To(BeEquivalentTo(map[interface{}]interface{}{}))
		...
		result, err = client.TSMRevRangeArgs(ctx, 0, 3, []string{"Test=This"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
		...
		// Test Align
		mrangeOpt = &redis.TSMRevRangeOptions{Aggregator: "count", BucketDuration: 10, Align: "-"}
		result, err = client.TSMRevRangeArgs(ctx, 0, 10, []string{"team=ny"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(result["a"][2]).To(BeEquivalentTo([]interface{}{[]interface{}{int64(10), 1.0}, []interface{}{int64(0), 10.0}}))

		mrangeOpt = &redis.TSMRevRangeOptions{Aggregator: "count", BucketDuration: 10, Align: 1}
		result, err = client.TSMRevRangeArgs(ctx, 0, 10, []string{"team=ny"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
	...
	It("should TSMRevRangeArgs Latest", Label("timeseries", "tsmrevrangeargs", "tsmrevrangelatest"), func() {
		resultCreate, err := client.TSCreate(ctx, "a").Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(resultCreate).To(BeEquivalentTo("OK"))
		opt := &redis.TSOptions{Labels: map[string]string{"is_compaction": "true"}}
		...
		result, err := client.TSMRevRangeArgs(ctx, 0, 10, []string{"is_compaction=true"}, mrangeOpt).Result()
		Expect(err).NotTo(HaveOccurred())
