...
func (c *clusterNodes) logf(ctx context.Context, format string, args ...interface{}) {
...
func (c *clusterState) slotClosestNode(slot int) (*clusterNode, error) {
	...
	if minLatency < maximumNodeLatency && closestNode != nil {
		c.nodes.logf(context.TODO(), "redis: all nodes are marked as failed, picking the temporarily failing node with lowest latency")
		return closestNode, nil
	}

	// If all nodes are having the maximum latency(all pings are failing) - return a random node across the cluster
	c.nodes.logf(context.TODO(), "redis: pings to all nodes are failing, picking a random node across the cluster")
	return c.nodes.Random()
}
...
func (c *ClusterClient) cmdInfo(ctx context.Context, name string) *CommandInfo {
	...
	if err != nil {
		c.logf(context.TODO(), "getting command info: %s", err)
		return nil
	}

	info := cmdsInfo[name]
	if info == nil {
		c.logf(context.TODO(), "info for cmd=%s not found", name)
	}
	return info
}
...
func (c *ClusterClient) logf(ctx context.Context, format string, args ...interface{}) {
	logger := internal.Logger
