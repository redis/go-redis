...
type FTHybridOptions struct {
	...
	Limit             int
	Params            map[string]any      // Parameter substitution
	...
	if cmd.val != nil {
		...
		if cmd.val.Rows != nil {
			...
			for i, row := range cmd.val.Rows {
				...
				if row.Fields != nil {
					val.Rows[i].Fields = make(map[string]any, len(row.Fields))
...
func (cmd *FTSearchCmd) Clone() Cmder {
	...
	if cmd.options != nil {
		...
		if cmd.options.InKeys != nil {
			options.InKeys = make([]any, len(cmd.options.InKeys))
			copy(options.InKeys, cmd.options.InKeys)
		}
		if cmd.options.InFields != nil {
			options.InFields = make([]any, len(cmd.options.InFields))
		...
		if cmd.options.Params != nil {
			options.Params = make(map[string]any, len(cmd.options.Params))
...
func newFTHybridCmd(ctx context.Context, options *FTHybridOptions, args ...any) *FTHybridCmd {
	var withCursor bool
...
func (cmd *FTHybridCmd) RawVal() any {
	return cmd.rawVal
}

func (cmd *FTHybridCmd) RawResult() (any, error) {
	return cmd.rawVal, cmd.err
}

func parseFTHybrid(data []any, withCursor bool) (FTHybridResult, *FTHybridCursorResult, error) {
	// Convert to map
	...
	resultsData, ok := resultMap["results"].([]any)
	if !ok {
		return FTHybridResult{}, nil, fmt.Errorf("invalid results format")
	}

	// Parse each result item
	results := make([]map[string]any, 0, len(resultsData))
	for _, item := range resultsData {
		// Try parsing as map[string]any first (RESP3 format)
		if itemMap, ok := item.(map[string]any); ok {
			results = append(results, itemMap)
			continue
		}

		// Try parsing as map[any]any (alternative RESP3 format)
		if rawMap, ok := item.(map[any]any); ok {
	...
	var warnings []string
	if warningsData, ok := resultMap["warnings"].([]any); ok {
...
func (cmd *FTHybridCmd) Clone() Cmder {
	...
	if cmd.val.Results != nil {
		val.Results = make([]map[string]any, len(cmd.val.Results))
		for i, result := range cmd.val.Results {
			val.Results[i] = make(map[string]any, len(result))
	...
	if cmd.options != nil {
		...
		if cmd.options.GroupBy != nil {
			...
			if cmd.options.GroupBy.ReduceParams != nil {
				options.GroupBy.ReduceParams = make([]any, len(cmd.options.GroupBy.ReduceParams))
