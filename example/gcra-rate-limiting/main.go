...
func main() {
	...
	fmt.Println("Example 1: Basic rate limiting (10 tokens/second, burst of 5)")
...
func handleRateLimitExceeded(ctx context.Context, rdb *redis.Client) {
	...
	// Allow only 1 token per second with no burst
	for i := 1; i <= 3; i++ {
...
func multipleUsers(ctx context.Context, rdb *redis.Client) {
	...
	for _, userKey := range users {
		// Allow 5 tokens per second with burst of 2
	...
	fmt.Printf("\n  user:bob still has %d tokens available\n", result.AvailableTokens)
}
