...
3. **Prioritize.** Order failures by what blocks a merge most: `build` and `test-e2e` first, then `golangci-lint` and `govulncheck`, then `codeql` / `doctests` / `spellcheck`, then everything else (benchmarks last).
...
For continuous watching: `/loop 5m /check-ci` polls this PR every 5 min until all checks pass. Run `/check-ci` alone for a one-shot status.
