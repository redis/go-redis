...
// BenchmarkOTelOverhead measures the performance overhead of OTel observability
// This benchmark compares two scenarios:
// 1. OTel Enabled - Full metrics collection
// 2. OTel Disabled - Feature code present but disabled (measures dormant code cost)
//
// To compare with baseline (no OTel code), run the benchmarks in the root package
// on upstream/master using the compare_perf.sh script
func BenchmarkOTelOverhead(b *testing.B) {
	...
	for _, scenario := range scenarios {
		b.Run(scenario.name, func(b *testing.B) {
			...
			// Create a no-op Prometheus exporter (metrics are collected but not exported)
			exporter, err := prometheus.New()
			if err != nil {
				b.Fatalf("Failed to create Prometheus exporter: %v", err)
			}
			provider := metric.NewMeterProvider(metric.WithReader(exporter))

			// Initialize OTel with enabled/disabled flag
			if err := redisotel.Init(rdb,
				redisotel.WithEnabled(scenario.enableMetrics),
				redisotel.WithMeterProvider(provider),
			); err != nil {
				b.Fatalf("Failed to initialize OTel: %v", err)
			}
...
// BenchmarkOTelOverhead_ConnectionPool measures connection pool overhead with high concurrency
