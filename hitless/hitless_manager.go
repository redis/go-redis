...
// CompleteOperationWithConnID completes a MOVING operation with a specific connection ID.
func (hm *HitlessManager) CompleteOperationWithConnID(seqID int64, connID uint64) {
	hm.mu.Lock()
	defer hm.mu.Unlock()
...
// GetState returns current state (alias for GetCurrentState for compatibility).
func (hm *HitlessManager) GetState() State {
	return hm.GetCurrentState()
}
...
func (hm *HitlessManager) CreateConnectionProcessor(protocol int, baseDialer func(context.Context, string, string) (net.Conn, error)) interfaces.ConnectionProcessor {
